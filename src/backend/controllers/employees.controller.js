...
exports.deactivateEmployee = (req, res) => {
    ...
    svc.deactivateEmployee(employeeId, (err, result) => {
        if (err) {
            console.error('Database error occurred deactivating employee:', err);
...
exports.reactivateEmployee = (req, res) => {
    const employeeId = req.params.id;
    if (!employeeId) return res.status(400).json({ error: 'Employee ID is required' });

    svc.reactivateEmployee(employeeId, (err, result) => {
        if (err) {
            console.error('Database error occurred reactivating employee:', err);
            return res.status(500).json({ error: 'Failed to reactivate employee.' });
        }
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: `Employee with ID ${employeeId} not found` });
        }
        return res.status(200).json({ message: 'Employee reactivated successfully.' });
    });
};
