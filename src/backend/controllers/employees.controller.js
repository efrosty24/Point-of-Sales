const svc = require('../services/employees.service');

// Simple add employee with given data
exports.addEmployee = (req, res) => {
    const employeeData = req.body;
    if (!employeeData || !employeeData.Email || !employeeData.Phone) {
        return res.status(400).json({error: 'Missing employee data or required fields.'});
    }
    svc.addEmployee(employeeData, (err, result) => {
        if (err) return res.status(500).json({ error: 'DB error'});

        return res.status(201).json({
            message : "Employee Added",
            employee: {
                EmployeeID: result.insertId,
                ...employeeData,
            }
        });
    });
};

// Dynamic way to get employees with filtering, will default to get all employees without filters
exports.getEmployees = (req, res) => {
    // Collect potential filter/sort options from the query string (e.g., /employees?role=Cashier&orderBy=name)
    const options = {
        employeeId: req.query.id,
        name: req.query.name,
        role: req.query.role,
        orderBy: req.query.sort, // Frontend might pass 'name', 'id', or 'role'
        orderDirection: req.query.dir // Frontend might pass 'ASC' or 'DESC'
    };

    // Determine which service function to call
    // If no specific filters are applied, you could call getAllEmployees for simplicity,
    // but getFilteredEmployees is flexible enough to handle both.

    svc.getFilteredEmployees(options, (err, rows) => {
        if (err) {
            console.error('Database error fetching employees:', err);
            return res.status(500).json({ error: 'Failed to retrieve employee data.' });
        }
        // Return the list of employees as JSON
        return res.status(200).json({
            message: `${rows.length} employees found.`,
            employees: rows
        });
    });
};

// Simple Update Employees Controller
exports.updateEmployee = (req, res) => {
    const employeeId = req.params.id;
    const updateData = req.body;

    // Ensure there is EmployeeID specified and body not empty
    if (!employeeId) {
        return res.status(400).json({error: 'Employee ID is required'});
    }
    if(Object.keys(updateData).length === 0){
        return res.status(400).json({error: 'No update data is provided'});
    }

    svc.updateEmployee(employeeId, updateData, (err, result) => {
        if (err && err.message.includes("No valid fields")) {
            return res.status(400).json({ error: err.message });
        }

        if (err) {
            console.error('Error occurred updating employee:', err);
            return res.status(500).json({ error: 'Failed to update employee due to server error.' });
        }

        if (result.affectedRows === 0) {
            return res.status(404).json({error: `Employee with ID ${employeeId} not found`});
        }

        return res.status(200).json({
            message: `${employeeId} updated successfully`,
            updatedFields: updateData
        });
    });
};

// Delete Employee Controller
exports.deleteEmployee = (req, res) => {
    const employeeId = req.params.id;

    if (!employeeId) {
        return res.status(400).json({error: 'Employee ID is required'});
    }

    svc.deleteEmployee(employeeId, (err, result) => {
        if (err) {
            console.error('Database error occurred deleting employee:', err);
            return res.status(500).json({ error: 'Failed to delete employee data.' });
        }

        if (result.affectedRows  === 0) {
            return res.status(404).json({error: `Employee with ID ${employeeId} not found`});
        }

        return res.status(204).send();
    });
};